...
const featuredThemes = computed(() => {
  ...
  return Object.keys(latestData.value.themes).filter(t => t !== 'General Development').slice(0, 5)
})
      ...
      <div v-if="latest" class="featured" :class="isWork ? 'featured-work' : 'featured-daily'" @click="router.push(`/${journal}/${latest.date}`)">
        ...
        <div class="feat-bottom">
          ...
          <div class="feat-volume" v-if="isWork && (linesAdded || linesRemoved)">
            <span class="vol-item vol-add">+{{ linesAdded.toLocaleString() }}</span>
            <span class="vol-item vol-del">-{{ linesRemoved.toLocaleString() }}</span>
            <span class="vol-item vol-files">{{ filesChanged }} files</span>
          </div>
        </div>
      </div>
...
/* Featured — unified layout */
.featured {
  border-radius: 14px; padding: 24px; margin-bottom: 16px;
  cursor: pointer; transition: border-color 0.2s;
}
.featured-work { background: rgba(99,149,255,0.03); border: 1px solid rgba(99,149,255,0.15); }
.featured-work:hover { border-color: rgba(99,149,255,0.3); }
.featured-daily { background: rgba(52,211,153,0.03); border: 1px solid rgba(52,211,153,0.15); }
.featured-daily:hover { border-color: rgba(52,211,153,0.3); }

.feat-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.feat-label { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.15em; color: var(--text-muted); }
.feat-stats-row { display: flex; align-items: center; gap: 16px; }
.fstat { display: flex; align-items: baseline; gap: 4px; }
.fstat-val { font-size: 16px; font-weight: 700; color: var(--text-heading); font-variant-numeric: tabular-nums; }
.fstat-lbl { font-size: 10px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; }
.feat-arrow { color: var(--text-muted); margin-left: 8px; }

.feat-date { font-family: var(--serif); font-size: 28px; font-weight: 400; color: var(--text-heading); margin-bottom: 10px; }
.feat-summary { font-size: 13px; color: var(--text); line-height: 1.65; margin-bottom: 16px; }
...
.feat-themes { display: flex; flex-wrap: wrap; gap: 5px; }
.ftag { font-size: 11px; color: var(--text); padding: 4px 10px; border-radius: 6px; border: 1px solid var(--border); background: var(--bg-elevated); }
.feat-volume { display: flex; align-items: center; gap: 10px; }
.vol-item { font-size: 12px; font-weight: 600; font-variant-numeric: tabular-nums; }
.vol-add { color: #34d399; }
.vol-del { color: #f87171; }
.vol-files { color: var(--text-muted); font-weight: 400; }
...
.sbar-work { background: rgba(99,149,255,0.45); }
.sbar-daily { background: rgba(52,211,153,0.45); }
...
.mdot-work { background: #6395ff; border: 2px solid rgba(99,149,255,0.3); }
.mdot-daily { background: #34d399; border: 2px solid rgba(52,211,153,0.3); }
...
.edot-work { background: rgba(99,149,255,0.5); border: 1.5px solid rgba(99,149,255,0.3); }
.edot-daily { background: rgba(52,211,153,0.5); border: 1.5px solid rgba(52,211,153,0.3); }
...
@media (max-width: 640px) {
  .feat-top { flex-direction: column; align-items: flex-start; gap: 8px; }
  .feat-bottom { flex-direction: column; align-items: flex-start; gap: 10px; }
